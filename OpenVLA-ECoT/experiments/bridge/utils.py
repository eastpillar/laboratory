...
#     unnorm_key = "bridge_orig" if "bridge_orig" in vla.norm_stats else "libero_goal"
#     action = vla.predict_action(image, task_label, unnorm_key=unnorm_key, do_sample=False, **kwargs)
#     return action
def get_vla_action(vla, obs, task_label, processor=None, info_dict=None, **kwargs):
    image = Image.fromarray(obs["full_image"]).convert("RGB")

    # -------------------------------------------------------------------------
    # [수정 1] 외부에서 전달된 캐시 모드 변수 받기
    # -------------------------------------------------------------------------
    # use_reasoning: True면 새로 생각함 (느림), False면 행동만 함 (빠름)
    # cached_reasoning: 이전에 했던 생각 텍스트
    use_reasoning = kwargs.pop("use_reasoning", True)
    cached_reasoning = kwargs.pop("cached_reasoning", None)
    ...
    # 2. 프롬프트 및 토큰 길이 결정 (핵심 로직)
    if use_reasoning:
        # [느린 모드] 생각을 처음부터 해야 함 -> 프롬프트는 그대로, 토큰은 길게
        prompt = base_prompt
        # 외부에서 1024를 안 줬을 경우를 대비해 기본값 설정
        run_max_tokens = kwargs.pop("max_new_tokens", 1024)
    else:
        # [빠른 모드] 아까 했던 생각을 입력에 포함시킴 -> 모델은 바로 행동만 뱉음
        # cached_reasoning이 없으면 그냥 base_prompt 사용 (안전장치)
        if cached_reasoning:
            prompt = f"{base_prompt} {cached_reasoning}"
        else:
            prompt = base_prompt
    ...
    assert processor is not None, "OpenVLA 사용 시 processor가 필요합니다."
    inputs = processor(prompt, image).to(vla.device, dtype=vla.dtype)

    # 나머지 kwargs 정리
    passed_unnorm_key = kwargs.pop("unnorm_key", None)
    if passed_unnorm_key is not None:
        unnorm_key = passed_unnorm_key
    else:
        unnorm_key = "bridge_orig" if "bridge_orig" in vla.norm_stats else "libero_goal"

    if "processor" in kwargs: del kwargs["processor"]
    if "info_dict" in kwargs: del kwargs["info_dict"]
    ...
    action, generated_ids = vla.predict_action(
        **inputs,
        ...
        do_sample=False,
        max_new_tokens=run_max_tokens,  # 위에서 결정한 토큰 길이 사용
        **kwargs,
    )

    if info_dict is not None and processor is not None:
        if use_reasoning:
            decoded_text = processor.batch_decode(generated_ids, skip_special_tokens=True)[0]
            trigger_str = "ASSISTANT: TASK:"
            if trigger_str in decoded_text:
                reasoning_text = decoded_text.split(trigger_str)[-1].strip()
            else:
                reasoning_text = decoded_text

            info_dict["decoded_tokens"] = reasoning_text
            print(f"\n[New Thought]: {reasoning_text}...")
        else:
            info_dict["decoded_tokens"] = cached_reasoning
