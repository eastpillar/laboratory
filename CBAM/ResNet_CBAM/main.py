...
from CBAM_network import ResNet50
import torch
...
#데이터 불러오기
train_list_path = '/바탕화면/adj_test/DenseNet/train_img_dir.txt'
test_list_path = '/바탕화면/adj_test/DenseNet/test_img_dir.txt'
...
model.load_state_dict(torch.load('/바탕화면/adj_test/ResNet/CBAM_save/150_model.pt', map_location=DEVICE))
if restore:
...
for epoch in range(1,EPOCHS+1):
    ...
    if optim_lr != check_lr or epoch == 1:
        ...
        heatM = False
    epochs = epoch * 10
    #model.load_state_dict(torch.load(f'/바탕화면/adj_test/ResNet/model_save/{epochs}_model.pt'))
