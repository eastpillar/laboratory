...
#학습 코드
# pretrained_model = 'model_save/200_model.pt' #enter pt file path.
    ...
    with torch.no_grad():
        for bth in tqdm(range(len(test_data))):
            ...
            for i in range(feature.shape[1]):
                ...
                cv2.imwrite(f"/VGG_features/feature_{i}.png", each_feature.astype(np.uint8)) # Feature save path
...
for epoch in range(1,EPOCHS+1):
    ...
    print("test done")
    with open(f'./model_accuracy/{exp_name}.txt' , 'a') as f: # Accuracy log file path
...
#     with open(f'./model_accuracy/{test_pt_file}.txt' , 'a') as f:
