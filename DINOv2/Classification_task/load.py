...
def data_loader(train_list_path, test_list_path):
    ...
    # train_images_arr = []
    # train_images = np.zeros((207005, 140, 140, 3), dtype=np.uint8)
    # train_images_gt = np.zeros(207005, dtype=np.uint8)
    # test_images = np.zeros((51752, 140, 140, 3), dtype=np.uint8)
    # test_images_gt = np.zeros(51752, dtype=np.uint8)
    test_image_name = []

    with open(train_list_path, 'r') as train_list:
        train_line_num = train_list.readlines()
        train_images = np.zeros((len(train_line_num), 140, 140, 3), dtype=np.uint8)
        train_images_gt = np.zeros(len(train_line_num), dtype=np.uint8)
        i = 0
        while True:
            ...
            img = cv2.resize(img,(140,140), interpolation=cv2.INTER_LINEAR)
    ...
    with open(test_list_path, 'r') as test_list:
        test_line_num = test_list.readlines()
        test_images = np.zeros((len(test_line_num), 140, 140, 3), dtype=np.uint8)
        test_images_gt = np.zeros(len(test_line_num), dtype=np.uint8)
        j = 0
        while True:
            ...
            labels = cv2.resize(labels, (140, 140), interpolation=cv2.INTER_LINEAR)
...
def mini_batch(data, gt, batch):
    imgs = torch.zeros((batch, 140, 140, 3), dtype=torch.float32)
    ...
    for idx,i in enumerate(data):
        ...
        if np.random.randint(2):
            ...
            cropped_img = img[x_start:x_start+140, y_start:y_start+140, :]
            img = cropped_img

        # rand_x = np.random.randint(0, 32, size=(2,))
        # rand_y = np.random.randint(0, 32, size=(2,))
        # large_crop1, large_crop2 = img[rand_x[0]:rand_x[0]+96, rand_y[0]:rand_y[0]+96, :], img[rand_x[1]:rand_x[1]+96, rand_y[1]:rand_y[1]+96, :]
        # crop_images = np.stack((large_crop1, large_crop2), axis=0)
