...
class ViT(nn.Module):
    def __init__(self, image_size, patch_size, hidden_dim, heads, mlp_dim, layers, dropout):
        ...
        self.pos_embed = nn.Parameter(torch.randn(1, num_patchs + 1, hidden_dim))
    ...
    def forward(self,x):
        out = self.patch_embed(x) #[batch,64,768]
        batch, n, _ = out.shape
        cls_token = repeat(self.cls_token, '1 1 d -> b 1 d', b=batch) #[batch,65,768]
        out = torch.cat((cls_token, out), dim=1)  #[batch,65,768]
        out += self.pos_embed[:, :(n + 1)]
        out = self.drop(out)
        # out = self.flatten_patch(x)
        out1 = self.transformer(out)

        out = self.mlp_head(out1[:,0,:])
        return out1[:,1:,:]
