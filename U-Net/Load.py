...
def data_loader(train_file, test_file):
    img_path = r"\VOCtrainval_11-May-2012\VOCtrainval_11-May-2012\VOCdevkit\VOC2012\JPEGImages"
    gt_img_path = r"\VOCtrainval_11-May-2012\VOCtrainval_11-May-2012\VOCdevkit\VOC2012\SegmentationClassAug"
    train_imgs_path, train_gts_path = [], []
    train_data = np.zeros((10582, 256, 256, 3), dtype=np.float32)
    train_gt_arr = np.zeros((10582, 256, 256), dtype=np.int64)
    test_img_arr = np.zeros((1449, 256, 256, 3), dtype=np.float32)
    test_gt_arr = np.zeros((1449, 256, 256), dtype=np.int64)
    ...
    with open(train_file, 'r') as img_name:
        ...
        for path in tqdm(range(len(train_imgs_path))):
